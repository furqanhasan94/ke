    ...
    private void lsPriorityChecker(Calendar currentTime){
        try {
            ...
            for (LsPriority event : lsPriorityEventList) {
                System.out.println("------Processing event number " + event.getId() + "------");
                if(event.geteDate().getYear() <= currentTime.getTime().getYear() 
                        && event.geteDate().getMonth() <= currentTime.getTime().getMonth()
                        && event.geteDate().getDate() <= currentTime.getTime().getDate()){
                            if( event.geteTime().getHours() <= currentTime.getTime().getHours() 
                                    && event.geteTime().getMinutes() <= currentTime.getTime().getMinutes()){
                                ...
                                + (event.geteTime().getHours()- currentTime.getTime().getHours()) +
                                        " hours are remaining");
                                }
                        }else{
                            System.out.println("In Priority event number " + event.getId() + "," 
                            + (event.geteDate().getDate()- currentTime.getTime().getDate()) + " days are remaining, "
                            + (event.geteDate().getMonth()- currentTime.getTime().getMonth() + " months are remaining, ")
                            + (event.geteDate().getYear()- currentTime.getTime().getYear()) + " years are remaining");
                        }
            }
        } catch (Exception e) {
